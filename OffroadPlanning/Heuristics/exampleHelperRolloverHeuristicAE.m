function cost = exampleHelperRolloverHeuristicAE(rsSegment, costMap, weight)
%exampleHelperRolloverHeuristicAE Penalize motion with transverse
%gradient for given Reeds-Shepp path segment

% Copyright 2023 The MathWorks, Inc.

    states = rsSegment.interpolate(0:1:rsSegment.Length);
        
    xyPoints = states(:,1:2);
    thetas   = states(:,3);

    dXY = abs(xyPoints(2:end,:)-xyPoints(1:end-1,:));
    transitionCost = sum(sqrt(sum(dXY.^2,2)));

    xyPoints = xyPoints(1:end-1,:);

    % XY flipped to calculate perpendicularity
    vGrad = -tan(thetas(2:end)-thetas(1:end-1)).*[getMapData(costMap,"yCost",xyPoints) getMapData(costMap,"xCost",xyPoints)];

    % Calculate a cost based on transverse slope
    rolloverCost = sum(weight.*abs(dXY/norm(dXY).*vGrad),"all");

    if isnan(rolloverCost)
        rolloverCost = 0;
    end

    % Combine transit/rollover costs
    cost = transitionCost + rolloverCost;
end
